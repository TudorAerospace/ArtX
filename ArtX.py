...
class App:
    ...
    def paint(self, event ): #Functia care se activeaza cand dai click/drag ca sa pictezi
        if self.brush_type == "brush" or self.brush_type == "eraser":   #pentru pensula
            ...
            if self.brush_shape == "ellipse":
                ...
                self.CVS.create_line( self.x0, self.y0, self.x1, self.y1, fill = self.Colour,  tags=f"brush{self.brush_num}" )
                self.draw.line([self.x0, self.y0, self.x1, self.y1], fill=self.Colour2)
                self.image_states.append((f"brush{self.brush_num}", self.x0, self.y0, self.x1, self.y1, "line", self.Colour, self.Colour2))
                self.x0, self.y0, self.x1, self.y1 = None, None, None, None
        else: #pentru forme
    ...
    def create_line(self, x0, y0, x1, y1):
        ...
        self.image_states.append((tag, x0, y0, x1, y1, "line", self.Colour, self.Colour2))
        self.line_num += 1
        self.CVS.create_line(x0, y0, x1, y1, fill=self.Colour, tags=tag)
        self.draw.line([x0, y0, x1, y1], fill=self.Colour2)
    ...
    def revert_state(self):
        ...
        if self.image_states:
            ...
            self.CVS.delete("all")
            self.image = Image.new("RGB", (self.CVS_width, self.CVS_height), self.bkgr_clr)
            self.draw = ImageDraw.Draw(self.image)
            ...
            for state in self.image_states:
                ...
                if tag.startswith("background"):
                    ...
                    self.CVS.create_line(x0, y0, x1, y1, fill=clr, tags=tag)
                    self.draw.line([x0, y0, x1, y1], fill=clr2)
    ...
    def open_achievement_window(self):
        ...
        self.achievement_3.grid(row=2, column=0, pady=5)
        ...
        self.achievement_4.grid(row=3, column=0, pady=5)
    ...
    def handle_achievements(self, action):
        ...
        if action == 0:
            with open(f"{self.artx_directory}achievement_progress.txt", "r") as self.achievement_progress:
            ...
            with open(f"{self.artx_directory}achievement_progress.txt", "w") as self.achievement_progress:
                for key, value in self.achievements.items():
                    comment = f"# {key} - {value}, Original Comment"
                    self.achievement_progress.write(f"{comment}\n")
                    self.achievement_progress.write(f"{value}\n")
    ...
    def draw_label(self):   #Text + Notes
        label_test_clr = Label(self.WIN, text="Compare Colours", font=("Arial", 16, "bold"), fg="black", bg="white")
