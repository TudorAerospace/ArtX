...
class App:
    def __init__(self):
        ...
        self.v_sub_menu = Menu(self.menu, tearoff= 0)
    ...
    def filter(self, _type): 
        if self.achievements['ach_8'] == 0:
            ...
            self.handle_achievements(1)
        if _type == "bnw" or _type == "black and white":
            self.image = self.image.convert("1")
            self.Colour = 'black'
            self.Colour2 = 0
        elif _type == "grs" or _type == "grayscale":
            self.image = self.image.convert("L")
            self.Colour = 'black'
            self.Colour2 = 0
        elif _type == None or _type == "RGB":
            self.image = self.image.convert("RGB")
            self.Colour = 'black'
            self.Colour2 = 0
        elif _type == 'uex' or _type == "underexpose":
            self.brightness_factor = 0.5
            enhancer = ImageEnhance.Brightness(self.image)
            self.image = enhancer.enhance(self.brightness_factor)
            self.brightness_factor = 1.0
        elif _type == 'oex' or _type == "overexpose":
            self.brightness_factor = 1.5
            enhancer = ImageEnhance.Brightness(self.image)
            self.image = enhancer.enhance(self.brightness_factor)
            self.brightness_factor = 1.0
        elif _type == "spn" or _type == "sharpen":
    ...
    def draw_menu(self):    #Creaza butoanele din meniu
        ...
        tool_menu.add_command(label="Enable Voice", command= lambda: self.start_recognition_thread())
    ...
    def help_voice_commands(self):
        if self.voice_commands_open == False:
            self.voice_commands_open = True
            self.VCM = Toplevel(self.WIN)
            self.VCM.attributes('-topmost', True)
            self.VCM.title("Voice Commands")
            self.VCM.resizable(False, False)
            self.update_window_position()
            label = Label(self.VCM, text="List of Voice Commands")
            label.grid(row=0, column=0)
            label = Label(self.VCM, text="<Select Brush> - Switches Paint Tool to Brush", anchor='w', justify='left')
            label.grid(row=1, column=0, sticky='w')
            label = Label(self.VCM, text="<Select Eraser> - Switches Paint Tool to Eraser", anchor='w', justify='left')
            label.grid(row=2, column=0, sticky='w')        
            label = Label(self.VCM, text="<Select Rectangle> - Switches Shape to Rectangle", anchor='w', justify='left')
            label.grid(row=3, column=0, sticky='w')   
            label = Label(self.VCM, text="<Select Ellipse> - Switches Shape to Ellipse", anchor='w', justify='left')
            label.grid(row=4, column=0, sticky='w')      
            label = Label(self.VCM, text="<Select Triangle> - Switches Shape to Triangle", anchor='w', justify='left')
            label.grid(row=5, column=0, sticky='w')         
            label = Label(self.VCM, text="<Select Line> - Switches Shape to Line", anchor='w', justify='left')
            label.grid(row=6, column=0, sticky='w')
            label = Label(self.VCM, text="<Brighten Image> - Increases Brightness by 10%", anchor='w', justify='left')
            label.grid(row=7, column=0, sticky='w')
            label = Label(self.VCM, text="<Darken Image> - Decreases Brightness by 10%", anchor='w', justify='left')
            label.grid(row=8, column=0, sticky='w')
            label = Label(self.VCM, text="<Revert/Back/Delete> - Reverts Action", anchor='w', justify='left')
            label.grid(row=9, column=0, sticky='w')
            label = Label(self.VCM, text="<New Image> - Opens New Image Dialog", anchor='w', justify='left')
            label.grid(row=10, column=0, sticky='w')
            label = Label(self.VCM, text="<Save Image> - Opens Save Image Dialog", anchor='w', justify='left')
            label.grid(row=11, column=0, sticky='w')
            label = Label(self.VCM, text="<Open/Load Image> - Opens Load Image Dialog", anchor='w', justify='left')
            label.grid(row=12, column=0, sticky='w')
            label = Label(self.VCM, text="<Get Idea> - Generates Drawing Idea", anchor='w', justify='left')
            label.grid(row=13, column=0, sticky='w')
            label = Label(self.VCM, text="<Choose Color> - Opens Color Picker Dialog", anchor='w', justify='left')
            label.grid(row=14, column=0, sticky='w')
            label = Label(self.VCM, text="<Fill Background> - Fills Background With Selected Color", anchor='w', justify='left')
            label.grid(row=15, column=0, sticky='w')     
            label = Label(self.VCM, text="<Close/Leave/Exit> - Opens Close App Dialog", anchor='w', justify='left')
            label.grid(row=16, column=0, sticky='w')         
            label = Label(self.VCM, text="<Pick> - Switches Tool to Color Picker", anchor='w', justify='left')
            label.grid(row=17, column=0, sticky='w')
            label = Label(self.VCM, text="<Zoom In> - Zooms In", anchor='w', justify='left')
            label.grid(row=18, column=0, sticky='w')
            label = Label(self.VCM, text="<Zoom Out> - Zooms Out", anchor='w', justify='left')
            label.grid(row=19, column=0, sticky='w')
            label = Label(self.VCM, text="<Extend Canvas> - Extends the Canvas", anchor='w', justify='left')
            label.grid(row=20, column=0, sticky='w')
            label = Label(self.VCM, text="<Apply Filter + (Filter)> - Applies a Filter", anchor='w', justify='left')
            label.grid(row=21, column=0, sticky='w')
            label = Label(self.VCM, text="Possible Filters:", anchor='w', justify='left')
            label.grid(row=22, column=0, sticky='w')
            label = Label(self.VCM, text="Black and White, Grayscale, RGB, Underexpose, Overexpose,", anchor='w', justify='left')
            label.grid(row=23, column=0, sticky='w')
            label = Label(self.VCM, text="Sharpen, Smooth, Blur, Contour, Detail, Edge Enhance", anchor='w', justify='left')
            label.grid(row=24, column=0, sticky='w')
            label = Label(self.VCM, text="<Open Achievement Window> - Opens Achievement Window", anchor='w', justify='left')
            label.grid(row=25, column=0, sticky='w')
            label = Label(self.VCM, text="<Start a Competition> - Opens the Competition Window", anchor='w', justify='left')
            label.grid(row=26, column=0, sticky='w')
            label = Label(self.VCM, text="<Disable Voice Commands> - Disables Voice Commans, Enable From Tools", anchor='w', justify='left')
            label.grid(row=27, column=0, sticky='w')
            self.VCM.protocol('WM_DELETE_WINDOW', self.vcm_close)

    def help_voice_info(self):
        if self.voice_commands_open == False:
            self.voice_commands_open = True
            self.VCI = Toplevel(self.WIN)
            self.VCI.attributes('-topmost', True)
            self.VCI.title("Voice Command Info")
            self.VCI.resizable(False, False)
            self.update_window_position()
            label = Label(self.VCI, text="What are Voice Commands?")
            label.grid(row=0, column=0)
            label = Label(self.VCI, text="ArtX makes use of voice commands to enhance user experience,", anchor='w', justify='left')
            label.grid(row=1, column=0, sticky='w')
            label = Label(self.VCI, text="streamline workflows, and make the app more accessible.", anchor='w', justify='left')
            label.grid(row=2, column=0, sticky='w')
            label = Label(self.VCI, text="Focus: ")
            label.grid(row=3, column=0)
            label = Label(self.VCI, text="Voice commands allow you to focus on the artwork, rather than", anchor='w', justify='left')
            label.grid(row=4, column=0, sticky='w')
            label = Label(self.VCI, text=" searching for tools or options.", anchor='w', justify='left')
            label.grid(row=5, column=0, sticky='w')
            label = Label(self.VCI, text="Enhanced User Experience:")
            label.grid(row=6, column=0)
            label = Label(self.VCI, text="Voice commands make interacting with the app feel more natural", anchor='w', justify='left')
            label.grid(row=7, column=0, sticky='w')
            label = Label(self.VCI, text="and intuitive, similar to having a painting assistant.", anchor='w', justify='left')
            label.grid(row=8, column=0, sticky='w')
            label = Label(self.VCI, text="Accessibility: ")
            label.grid(row=9, column=0)
            label = Label(self.VCI, text="Voice commands make ArtX more accessible, requiring less", anchor='w', justify='left')
            label.grid(row=10, column=0, sticky='w')
            label = Label(self.VCI, text="mouse movements to perform certain tasks", anchor='w', justify='left')
            label.grid(row=11, column=0, sticky='w')
            label = Label(self.VCI, text="Can I Disable Voice Commands? ")
            label.grid(row=12, column=0)
            label = Label(self.VCI, text="Yes! Simply say <Disable Voice Commands>. They can then be ", anchor='w', justify='left')
            label.grid(row=13, column=0, sticky='w')
    ...
    def vcm_close(self):
        self.voice_commands_open = False
        if self.VCM:
            self.VCM.destroy()
        else:
            self.VCI.destroy()
    ...
    def handle_voice_input(self, action):
        action = str(action).lower()
        if "brush" in action:
            self.set_type("brush")
        if "eraser" in action:
            self.eraser()
        if "rectangle" in action or "square" in action:
            self.set_type("rectangle")
        if "ellipse" in action or "circle" in action:
            self.set_type("ellipse")
        if "triangle" in action:
            self.set_type("triangle")
        if "line" in action:
            self.set_type("line")
        if "brighten" in action or "brighton" in action or "lighten" in action or "light on" in action:
            self.brightness(1)
        if "darken" in action or "dhadkan" in action:
            self.brightness(0)
        if "revert" in action or "back" in action or "delete" in action:
            self.revert_state()
        if "new" in action:
            self.open_new_file_window()
        if "save" in action:
            self.save_image_as(0)
        if "open" in action or "load" in action:
            self.open_image()
        if "idea" in action:
            self.get_idea()
        if "color" in action or "colour" in action:
            self.pick_color()
        if "fill" in action or "feel" in action or "phil" in action:
            self.paint_background()
        if "close" in action or "leave" in action or "exit" in action:
            self.on_closing()
        if "pick" in action:
            self.set_type("picker")
        if "zoom in" in action:
            self.handle_zoom(1)
        if "zoom out" in action:
            self.handle_zoom(0)
        if "extend" in action:
            self.extend_canvas()
        if "filter" in action:
            action = action.replace("edge enhance", "edge_enhance")
            filters = ["black and white", "grayscale", "RGB", "underexpose", "overexpose", "sharpen", "SMOOTH", "BLUR", "CONTOUR", "DETAIL", "EDGE_ENHANCE"]
            for i in filters:
                if i.lower() in action:
                    self.filter(i)
        if "achievement" in action or "achievements" in action:
            self.open_achievement_window()
        if "competition" in action or "contest" in action:
            self.competition()
        if "help" in action:
            self.help_voice_commands()
        if "disable" in action or "voice" in action:
            self.voice = False
            self.stop_recognition_thread()
